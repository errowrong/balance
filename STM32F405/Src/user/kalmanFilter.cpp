...
void KALMANFILTER::TaskInit(std::vector<float> _P, std::vector<float> _F, std::vector<float> _Q,
    ...
    uint8_t xhatSize, uint8_t uSize, uint8_t zSize, uint8_t autoAdjust, std::function<void()>fun)
{
    ...
    //memcpy(P, &_P[0], _P.size()* sizeFloat);
    //memcpy(F, &_F[0], _F.size() * sizeFloat);
    //memcpy(Q, &_Q[0], _Q.size() * sizeFloat);
    //memcpy(R, &_R[0], _R.size() * sizeFloat);
    //memcpy(H, &_H[0], _H.size() * sizeFloat);
    //memcpy(Z, &_Z[0], _Z.size() * sizeFloat);
    //
    //// 设置最小方差
    ////static float state_min_variance[3]{};
    //memcpy(state_min_variance, &_state_min_variance[0], _state_min_variance.size()* sizeFloat);
    ////state_min_variance[3] = { 0.03, 0.005, 0.1 };

    // // 开启自动调整
    //kf.UseAutoAdjustment = autoAdjust;


    ////static uint8_t measurement_reference[3]{};
    //memcpy(measurement_reference, &_measurement_reference[0], _measurement_reference.size()* sizeFloat);

    ////static float measurement_degree[3]{};
    //// 根据measurement_reference与measurement_degree生成H矩阵如下（在当前周期全部测量数据有效情况下）
    //memcpy(measurement_degree, &_measurement_degree[0], _measurement_degree.size()* sizeFloat);

    ////static float mat_R_diagonal_elements[3] = {};
    //memcpy(mat_R_diagonal_elements, &_mat_R_diagonal_elements[0], _mat_R_diagonal_elements.size()* sizeFloat);
    ...
    memcpy(kf.H_data, &_H[0], _H.size() * sizeFloat);
    memcpy(kf.z_data, &_Z[0], _Z.size() * sizeFloat);
    memcpy(kf.MeasurementMap, &_measurement_reference[0], _measurement_reference.size() * sizeFloat);
    memcpy(kf.MeasurementDegree, &_measurement_degree[0], _measurement_degree.size() * sizeFloat);
    memcpy(kf.MatR_DiagonalElements, &_measurement_degree[0], _measurement_degree.size() * sizeFloat);
    memcpy(kf.StateMinVariance, &_mat_R_diagonal_elements[0], _mat_R_diagonal_elements.size() * sizeFloat);

    StateUpdate = fun;
}

void KALMANFILTER::TaskUpdate()
{
    StateUpdate();
    Kalman_Filter_Update(&kf);
}

void KALMANFILTER::SensorUpdate(std::vector<float> sensorValue)
{
    for (size_t i = 0; i < sensorValue.size(); i++)
    {
        kf.MeasuredVector[i] = sensorValue[i];
    }
}

float* KALMANFILTER::Kalman_Filter_Update(KalmanFilter_t* kf)
{
    // 0. 获取量测信息
 ...
 * @param zSize 观测量维度
 */
void KALMANFILTER::Kalman_Filter_Init(KalmanFilter_t* kf, uint8_t xhatSize, uint8_t uSize, uint8_t zSize)
{
...
void KALMANFILTER::Kalman_Filter_Measure(KalmanFilter_t* kf)
{
    // 矩阵H K R根据量测情况自动调整
...
void KALMANFILTER::Kalman_Filter_xhatMinusUpdate(KalmanFilter_t* kf)
{
    if (!kf->SkipEq1)
    {
        if (kf->uSize > 0)
...
void KALMANFILTER::Kalman_Filter_PminusUpdate(KalmanFilter_t* kf)
{
    if (!kf->SkipEq2)
    {
        ...
        kf->MatStatus = Matrix_Add(&kf->temp_matrix, &kf->Q, &kf->Pminus);  //Pminus = temp_matrix + Q

    }
}
void KALMANFILTER::Kalman_Filter_SetK(KalmanFilter_t* kf)
{
    if (!kf->SkipEq3)
    {
...
void KALMANFILTER::Kalman_Filter_xhatUpdate(KalmanFilter_t* kf)
{
    if (!kf->SkipEq4)
    {
...
void KALMANFILTER::Kalman_Filter_P_Update(KalmanFilter_t* kf)
{
    if (!kf->SkipEq5)
    {
...
void KALMANFILTER::H_K_R_Adjustment(KalmanFilter_t* kf)
{
    kf->MeasurementValidNum = 0;
